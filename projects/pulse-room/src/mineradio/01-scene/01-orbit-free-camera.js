...
/* Upstream: https://github.com/XxHuberrr/Mineradio/blob/4abaa190de42c632365ae4244e041bad16443224/public/js/modules/01-scene/01-orbit-free-camera.js */
...
function resetFreeCameraToDefault() {
  ...
  freeCamera.resetTween = {
    ...
    duration: PulseRuntime.quality.cameraTravel === false ? 1 : 620,
    from: {
      position: fromPos,
...
function updateFreeCamera(dt) {
  ...
  if (PulseRuntime.quality.cameraTravel === false) {
    if (freeCamera.velocity) freeCamera.velocity.set(0, 0, 0);
    return;
  }
