...
/* Upstream: https://github.com/XxHuberrr/Mineradio/blob/4abaa190de42c632365ae4244e041bad16443224/public/js/modules/05-playback/08-audio-graph-controls.js */
/* Modified for PULSE ROOM on 2026-07-30. */
// ============================================================
function applyOptionalAudioOutputDevice(media) {
  if (typeof applyAudioOutputDevice !== 'function') return Promise.resolve(false);
  try {
    return Promise.resolve(applyAudioOutputDevice(media)).catch(function () { return false; });
  } catch (e) {
    return Promise.resolve(false);
  }
}

function pulseDirectFileAudio() {
  try {
    var src = audio && (audio.currentSrc || audio.src || '');
    return window.location && window.location.protocol === 'file:' && /^file:/i.test(src);
  } catch (e) {
    return false;
  }
}
...
function replaceAudioElementForGraphRecovery(reason, opts) {
  ...
  var loop = !!oldAudio.loop;
  var muted = !!oldAudio.muted;
  ...
  var mediaTrackSwitchToken = oldAudio.__mineradioTrackSwitchToken;
  ...
  audio.__mineradioTrackSwitchToken = mediaTrackSwitchToken;
  if (src) {
    audio.src = src;
    restoreMediaTimeWhenReady(audio, seconds);
  }
  if (window.PulseRuntime && window.PulseRuntime.playback && typeof window.PulseRuntime.playback.adoptMedia === 'function') {
    window.PulseRuntime.playback.adoptMedia(audio);
  }
  ...
  if (src && !wasPaused) {
    try {
      audio.load();
      window.PulseRuntime.playback.play().catch(function (e) {
        console.warn('replacement audio resume failed:', e && (e.message || e));
      });
    } catch (e) { }
  }
  applyOptionalAudioOutputDevice(audio);
...
function initAudio() {
  if (!audio) return false;
  // Chromium intentionally zeroes MediaElementSource output for file:// media
  // because the opaque file origin cannot satisfy CORS. Let the native audio
  // element stay on its direct output path; the bundled analysis bridge keeps
  // the visual signal alive without sacrificing audible playback.
  if (pulseDirectFileAudio()) return false;
...
function schedulePlaybackAnalyserRecovery(reason) {
  if (pulseDirectFileAudio()) return;
...
function setVolume(value, silent) {
  ...
  try { localStorage.setItem('pulse-room-volume-v3', String(next)); } catch (e) { }
  ...
  if (window.PulseRuntime && window.PulseRuntime.playback) {
    window.PulseRuntime.playback.setVolume(audio && isFinite(audio.volume) ? audio.volume : next);
  }
  updateVolumeUi();
