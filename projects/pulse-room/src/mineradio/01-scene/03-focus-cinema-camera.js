...
/* Upstream: https://github.com/XxHuberrr/Mineradio/blob/4abaa190de42c632365ae4244e041bad16443224/public/js/modules/01-scene/03-focus-cinema-camera.js */
...
function updateCamera() {
  ...
  if (PulseRuntime.quality.impactShake === false) cameraShake = 0;
...
function shouldUseCompactShelfCamera() {
  return PulseRuntime.quality.id === 'mobile';
}
...
function updateCinema(dt) {
  ...
  if (PulseRuntime.quality.impactShake === false) shake = 0;
  var beatDamp = (orbit.focus.active ? (dj ? 0.66 : 0.55) : (dj ? 1.12 : 1.0)) * shake;
  var idleDamp = damp * (dj ? 0.72 : 1.0) * shake;
  var reduced = PulseRuntime.quality.cameraTravel === false;
  if (reduced) idleDamp = 0;
  orbit.cineTheta = Math.sin(cinemaT * 0.08) * (reduced ? 0.002 : 0.012) * idleDamp + beatCam.thetaKick * beatDamp;
  orbit.cinePhi = Math.sin(cinemaT * 0.06 + 1.0) * (reduced ? 0.002 : 0.010) * idleDamp + beatCam.phiKick * beatDamp;
  orbit.cineRadius = Math.sin(cinemaT * 0.04 + 2.0) * (reduced ? 0.012 : 0.080) * idleDamp - beatCam.radiusKick * beatDamp * (dj ? 1.22 : 1.18);
}
updateCamera();
