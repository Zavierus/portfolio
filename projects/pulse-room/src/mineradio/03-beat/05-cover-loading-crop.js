...
/* Upstream: https://github.com/XxHuberrr/Mineradio/blob/4abaa190de42c632365ae4244e041bad16443224/public/js/modules/03-beat/05-cover-loading-crop.js */
...
function showAIDepthChip(text) {
  var textNode = document.getElementById('ai-depth-text');
  var chip = document.getElementById('ai-depth-chip');
  if (textNode) textNode.textContent = text || '封面景深处理中';
...
function applyCoverCanvas(canvas, thumbSrc, opts) {
  opts = opts || {};
  if (!canvas || !coverApplyStillCurrent(opts)) return false;
  var paletteTrackId = opts.trackId || (currentTrack && currentTrack.id) || opts.coverSource || 'pulse-room';
  if (typeof updateMusicSpacePaletteFromCanvas === 'function') {
    updateMusicSpacePaletteFromCanvas(canvas, paletteTrackId);
  }
  var thumb = document.getElementById('thumb-cover');
  if (thumbSrc && thumb) thumb.src = thumbSrc;
  if (typeof setControlCoverSrc === 'function') setControlCoverSrc(thumbSrc || '');
  if (shelfManager) shelfManager.onCoverChange(thumbSrc || '');
  return true;
}

function applyPulseCoverFallback(opts, failedSource, error) {
  if (!window.PulseRuntime || typeof PulseRuntime.createCoverFallback !== 'function') return false;
  if (!coverApplyStillCurrent(opts)) return false;
  var trackId = String(opts.trackId || (currentTrack && currentTrack.id) || failedSource || 'pulse-room');
  var bands = {
    low: typeof smoothBass === 'number' ? smoothBass : 0.54,
    mid: typeof smoothMid === 'number' ? smoothMid : 0.46,
    high: typeof smoothTreb === 'number' ? smoothTreb : 0.62
  };
  try {
    var canvas = PulseRuntime.createCoverFallback(trackId, bands);
    var fallbackSource = canvas.toDataURL ? canvas.toDataURL('image/png') : '';
    if (fallbackSource) setAlbumBackground(fallbackSource);
    applyCoverCanvas(canvas, fallbackSource, Object.assign({}, opts, {
      coverKey: 'fallback:' + trackId,
      coverSourceKind: 'fallback',
      coverSource: trackId
    }));
    if (PulseRuntime.status) {
      PulseRuntime.status.report('cover', { trackId: trackId, error: error });
    }
    return true;
  } catch (fallbackError) {
    if (PulseRuntime.status) {
      PulseRuntime.status.report('cover', { trackId: trackId, error: fallbackError });
    }
    return false;
  }
}
...
function loadCoverFromUrl(directUrl, opts) {
  ...
  if (!directUrl || typeof directUrl !== 'string' || !/^https?:\/\//i.test(directUrl)) {
    ...
    if (typeof updateMusicSpacePaletteFromCanvas === 'function') {
      updateMusicSpacePaletteFromCanvas(null, opts.trackId || (currentTrack && currentTrack.id) || 'pulse-room');
    }
    if (preserveOnSwitch) return;
    setAlbumBackground('');
  ...
  if (!proxiedUrl) {
    if (typeof updateMusicSpacePaletteFromCanvas === 'function') {
      updateMusicSpacePaletteFromCanvas(null, opts.trackId || (currentTrack && currentTrack.id) || directUrl);
    }
    if (preserveOnSwitch) return;
    setAlbumBackground('');
    setControlCoverSrc('');
  ...
  img.onload = function () {
    ...
    applyCoverCanvas(cv, proxiedUrl || directUrl, Object.assign({}, opts, { coverKey: opts.coverKey || directUrl || proxiedUrl || '', coverSourceKind: 'url', coverSource: directUrl }));
  };
  img.onerror = function () {
    ...
    img2.onload = function () {
      ...
      var size = coverTextureSizeForResolution(fx.coverResolution);
      var cv = document.createElement('canvas'); cv.width = cv.height = size;
      cv.getContext('2d', { willReadFrequently: true }).drawImage(img2, 0, 0, size, size);
      setAlbumBackground(directUrl);
      applyCoverCanvas(cv, directUrl, Object.assign({}, opts, { coverKey: opts.coverKey || directUrl || '', coverSourceKind: 'url', coverSource: directUrl }));
    };
    img2.onerror = function () {
      if (!coverApplyStillCurrent(opts)) return;
      if (applyPulseCoverFallback(opts, directUrl, new Error('Cover decode failed'))) return;
      if (preserveOnSwitch) return;
      setAlbumBackground('');
      setControlCoverSrc('');
    };
