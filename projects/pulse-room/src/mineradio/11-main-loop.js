...
/* Upstream: https://github.com/XxHuberrr/Mineradio/blob/4abaa190de42c632365ae4244e041bad16443224/public/js/modules/11-main-loop.js */
...
var lastPulseAnalysisFrame = null;
window.__PULSE_ROOM_QA__ = window.__PULSE_ROOM_QA__ || {};
window.__PULSE_ROOM_QA__.musicSpace = function () {
  var layerObjects = [spatialHaze, spectralRibbons, driftingFlow, transientGlints];
  var layers = layerObjects.map(function (layer) {
    return {
      name: layer.name,
      count: layer.geometry && layer.geometry.attributes.position
        ? Number(layer.geometry.attributes.position.count) || 0
        : 0,
      visible: !!layer.visible
    };
  });
  var memory = renderer.info && renderer.info.memory ? renderer.info.memory : {};
  var programs = renderer.info && renderer.info.programs ? renderer.info.programs.length : 0;
  return Object.freeze({
    tier: PulseRuntime.quality.id,
    expected: PulseRuntime.quality.space,
    layers: layers,
    layerNames: layers.map(function (layer) { return layer.name; }),
    counts: {
      spatialHaze: layers[0].count,
      spectralRibbons: layers[1].count,
      driftingFlow: layers[2].count,
      transientGlints: layers[3].count
    },
    fluidSoundstage: typeof fluidSoundstageState === 'function' ? fluidSoundstageState() : { visible: false, healthy: false, rippleCount: 0 },
    rootUuid: musicSpace.uuid,
    cameraUuid: camera.uuid,
    handoffProgress: musicSpaceHandoff.progress,
    analysisSerial: PulseRuntime.analysis.state.serial,
    reducedMotion: !!PulseRuntime.quality.reducedMotion,
    activeGlints: activeGlintCount,
    alpha: Number(uniforms.uAlpha.value),
    particleDim: Number(uniforms.uParticleDim.value),
    visible: !!musicSpace.visible && layers.every(function (layer) { return layer.visible; }),
    resources: {
      geometries: Number(memory.geometries) || 0,
      textures: Number(memory.textures) || 0,
      programs: Number(programs) || 0
    },
    cameraMode: orbit && orbit.focus && orbit.focus.active
      ? 'focus:' + String(orbit.focus.type || 'stage')
      : (fx.cinema ? 'cinema' : (orbit.rotating ? 'manual' : 'idle')),
    total: layers.reduce(function (sum, layer) { return sum + layer.count; }, 0)
  });
};
window.__PULSE_ROOM_QA__.particles = window.__PULSE_ROOM_QA__.musicSpace;
window.__PULSE_ROOM_QA__.analysis = function () {
  var frame = lastPulseAnalysisFrame || {};
  return Object.freeze({
    status: PulseRuntime.analysis.state.status,
    source: frame.source || PulseRuntime.analysis.state.source,
    serial: Number(frame.serial ?? PulseRuntime.analysis.state.serial) || 0,
    playing: !!(playing && audio && !audio.paused),
    currentTime: audio && isFinite(audio.currentTime) ? audio.currentTime : 0,
    visualTime: uniforms && uniforms.uTime ? uniforms.uTime.value : 0,
    low: Number(frame.low) || 0,
    mid: Number(frame.mid) || 0,
    high: Number(frame.high) || 0,
    peak: Number(frame.peak) || 0,
    onset: Number(frame.onset) || 0,
    cameraEvent: frame.cameraEvent || null,
    particleEvent: frame.particleEvent || null
  });
};
...
function animate() {
  ...
  if (isMainSceneCoveredBySplash()) {
    if (now - splashWarmRenderLast > 520) {
      ...
      if (!rendererSoftwareFallback || !rendererSoftwareFallbackRendered) {
        renderer.render(scene, camera);
        if (rendererSoftwareFallback) rendererSoftwareFallbackRendered = true;
      }
  ...
  var pulseAnalysisTime = audio && isFinite(audio.currentTime) ? Math.max(0, audio.currentTime) : uniforms.uTime.value;
  var pulseAnalysisFrame = PulseRuntime.analysis.sample({
    trackId: currentTrack && currentTrack.id ? currentTrack.id : null,
    currentTime: pulseAnalysisTime,
    playing: !!(playing && audio && !audio.paused),
    frameId: renderPerfState.frames,
    realtime: {
      low: bass,
      mid: mid,
      high: treble,
      peak: audioEnergy,
      onset: beatOnsetFlag ? beatPulse : 0,
      spectralCentroid: treble
    }
  });
  lastPulseAnalysisFrame = pulseAnalysisFrame;
  consumePulseAnalysisFrame(pulseAnalysisFrame);
  if (playing && audio && !audio.paused) {
    bass = pulseAnalysisFrame.low * fx.intensity;
    mid = pulseAnalysisFrame.mid * fx.intensity;
    treble = pulseAnalysisFrame.high * fx.intensity;
    audioEnergy = pulseAnalysisFrame.peak;
    beatPulse = pulseAnalysisFrame.onset;
  } else {
    beatPulse = 0;
  }
  ...
  updateUnifiedMusicSpace(dt, pulseAnalysisFrame, {
    playing: !audio.paused,
    analysisSerial: PulseRuntime.analysis.state.serial,
    reducedMotion: PulseRuntime.quality.reducedMotion
  });
  if (document.body) {
    var particleEvent = pulseAnalysisFrame.particleEvent;
    var particleImpact = particleEvent && isFinite(Number(particleEvent.intensity)) ? Number(particleEvent.intensity) : 0;
    var pulseEventValue = clamp01((pulseAnalysisFrame.onset || 0) * 1.8 + (pulseAnalysisFrame.peak || 0) * 0.25 + particleImpact * 0.45);
    document.body.style.setProperty('--pulse-energy', String(clamp01((pulseAnalysisFrame.energy || pulseAnalysisFrame.peak || 0) * 1.35)));
    document.body.style.setProperty('--pulse-bass', String(clamp01(pulseAnalysisFrame.low || 0)));
    document.body.style.setProperty('--pulse-event', String(pulseEventValue));
    document.body.style.setProperty('--pulse-flow-x', String(clampRange(mouseWorld.x || 0, -1, 1)));
    document.body.style.setProperty('--pulse-flow-y', String(clampRange(mouseWorld.y || 0, -1, 1)));
    if (!PulseRuntime.quality.reducedMotion && pulseEventValue > 0.72 && !document.body.classList.contains('pulse-impact')) {
      document.body.classList.add('pulse-impact');
      clearTimeout(window.__pulseImpactTimer);
      window.__pulseImpactTimer = setTimeout(function () { document.body.classList.remove('pulse-impact'); }, 180);
    }
  }
  ...
  var visualBackdropDim = sonicPresetActiveEarly ? 0.82 : 1;
  var shelfDimTarget = shouldDimWallpaperForShelf() ? 0.48 : visualBackdropDim;
  var fluidParticleTarget = typeof unifiedMusicSpaceState !== 'undefined'
    ? clampRange(0.28 + (unifiedMusicSpaceState.high || 0) * 0.24 + Math.max(unifiedMusicSpaceState.onset || 0, unifiedMusicSpaceState.peak || 0) * 0.38, 0.24, 1)
    : 1;
  shelfDimTarget = Math.min(shelfDimTarget, PulseRuntime.quality.reducedMotion ? 0.46 : fluidParticleTarget);
  ...
  var musicSpacePerfStart = performance.now();
  if (perfProbe && perfProbe.markSince) perfProbe.markSince('visual.music-space', musicSpacePerfStart);
  ...
  if (lyricsParticleStepDt > 0 && typeof tickLyricsParticles === 'function') tickLyricsParticles();
  ...
  if (homeAudioStepDt > 0 && typeof updateHomeAudioVisual === 'function') updateHomeAudioVisual(homeAudioStepDt);
  ...
  if (typeof tickGestureRotation === 'function') tickGestureRotation(dt);
  ...
  musicSpace.rotation.y += (targetRotY - musicSpace.rotation.y) * 0.055;
  musicSpace.rotation.x += (targetRotX - musicSpace.rotation.x) * 0.055;
  // 同步给背面粒子层
  ...
  if (window.MineradioSonicTopography) {
    MineradioSonicTopography.update(dt, {
      ...
      visualRotation: musicSpace.rotation,
  ...
  if (stageLyricsStepDt > 0 && typeof updateStageLyrics3D === 'function') updateStageLyrics3D(stageLyricsStepDt);
  ...
  if (desktopOverlayStepDt > 0 && typeof syncDesktopOverlayState === 'function') syncDesktopOverlayState();
  ...
  if (!rendererSoftwareFallback) {
    renderer.render(scene, camera);
  } else if (!rendererSoftwareFallbackRendered) {
    renderer.render(scene, camera);
    rendererSoftwareFallbackRendered = true;
  }
