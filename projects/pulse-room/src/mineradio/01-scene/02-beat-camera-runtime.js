...
/* Upstream: https://github.com/XxHuberrr/Mineradio/blob/4abaa190de42c632365ae4244e041bad16443224/public/js/modules/01-scene/02-beat-camera-runtime.js */
/* Modified for PULSE ROOM on 2026-07-30. */
function flushPersistentVisualState() {
  try { flushVisualSettingsSave(); } catch (e) { }
...
function scheduleBeatCamera(beat, source) {
  if (!fx.cinema) return;
  if (PulseRuntime.quality.impactShake === false) return;
...
function updateBeatCamera(dt) {
  ...
  thetaKick = clampRange(thetaKick, -0.045, 0.045);
  phiKick = clampRange(phiKick, -0.055, 0.055);
  radiusKick = clampRange(radiusKick, 0, 0.36);
  rollKick = clampRange(rollKick, -0.018, 0.018);
