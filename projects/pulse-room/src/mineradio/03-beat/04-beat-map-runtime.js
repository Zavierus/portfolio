...
/* Upstream: https://github.com/XxHuberrr/Mineradio/blob/4abaa190de42c632365ae4244e041bad16443224/public/js/modules/03-beat/04-beat-map-runtime.js */
/* Modified for PULSE ROOM on 2026-07-30. */
function consumePulseAnalysisFrame(frame) {
  if (!frame || !playing || !audio || audio.paused) return;
  if (frame.cameraEvent && !PulseRuntime.quality.reducedMotion) {
    scheduleBeatCamera({
      time: frame.cameraEvent.time,
      strength: frame.cameraEvent.intensity,
      confidence: 1,
      impact: frame.cameraEvent.intensity,
      primary: true,
      combo: frame.cameraEvent.type
    }, 'map');
  }
  if (frame.particleEvent) {
    var intensity = clamp01(frame.particleEvent.intensity);
    uniforms.uBurstAmt.value = Math.max(uniforms.uBurstAmt.value, intensity * (PulseRuntime.quality.reducedMotion ? 0.08 : 0.62));
    scheduledBeatPulse = Math.max(scheduledBeatPulse, intensity * (PulseRuntime.quality.reducedMotion ? 0 : 0.54));
    if (!PulseRuntime.quality.reducedMotion) scheduledBeatFlag = true;
  }
}
