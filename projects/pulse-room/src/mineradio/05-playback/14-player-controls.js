...
/* Upstream: https://github.com/XxHuberrr/Mineradio/blob/4abaa190de42c632365ae4244e041bad16443224/public/js/modules/05-playback/14-player-controls.js */
...
function isRemoteTrackStartStalled() {
  return false;
}

function remoteTrackStartNudgeSeconds(media) {
  var target = 0.22;
...
async function nudgeRemoteTrackStart(media, src, token) {
  ...
  try {
    ...
    if (!isSameAudioPlaybackTarget(media, src) || token !== trackSwitchToken || media.paused || media.ended) return false;
    var target = remoteTrackStartNudgeSeconds(media);
    ...
    console.warn('[PlaybackResumeRecovery] remote start nudge failed:', err && (err.message || err));
    return false;
  }
}

function remoteTrackStartResumeSeconds(media, current, startTime) {
  var target = Math.max(Number(current) || 0, Number(startTime) || 0, remoteTrackStartNudgeSeconds(media));
  var duration = media && isFinite(media.duration) ? Number(media.duration) : 0;
  if (duration > 0) target = Math.min(target, Math.max(0, duration - 0.75));
  return Math.max(0, target);
}

function showRemoteTrackStartStallNotice() {
  var now = performance.now();
  if (now - (playbackResumeRecovery.lastRemoteStartNoticeAt || 0) < 8000) return;
  playbackResumeRecovery.lastRemoteStartNoticeAt = now;
  var title = '音频播放未响应';
  var body = '音频开头解码卡住，请重试或选择其他曲目。';
...
function playbackFreshUrlRecoverySongKey(song) {
  ...
  return song ? String(song.id || song.mid || song.hash || '') : '';
}
...
function schedulePlaybackStallRecovery(reason, opts) {
  ...
  PLAYBACK_RESUME_STALL_DELAYS.forEach(function (delayMs) {
    var timerId = setTimeout(async function () {
      ...
      var remoteStartStall = isRemoteTrackStartStalled(song, opts, media, startTime, current);
      if (remoteStartStall && delayMs < 3000) {
        try {
          await ensurePlaybackAudioGraph('remote-start-stall-before-nudge');
          ensureAudiblePlaybackGain('remote-start-stall-before-nudge');
        } catch (nudgeGraphErr) {
          console.warn('[PlaybackResumeRecovery] remote graph precheck failed:', nudgeGraphErr);
        }
        if (!playbackStallRecoveryOwnerStillCurrent(media, src, token, recoverySerial, queueKey)) return;
        if (await nudgeRemoteTrackStart(media, src, token)) return;
        return;
      }
      ...
      if (current >= startTime + minAdvance) return;
      remoteStartStall = isRemoteTrackStartStalled(song, opts, media, startTime, current);
      var recovered = await recoverCurrentTrackPlaybackFromFreshUrl(remoteStartStall ? 'remote-track-start-stalled' : (reason || 'resume-stalled'), {
        resumeAt: remoteStartStall ? remoteTrackStartResumeSeconds(media, current, startTime) : (current || startTime),
        silent: opts.silent
      });
      if (!playbackStallRecoveryOwnerStillCurrent(media, src, token, recoverySerial, queueKey)) return;
      if (!recovered && remoteStartStall) showRemoteTrackStartStallNotice();
    }, delayMs);
...
function schedulePausedAudioResumeMaintenance(media, src, token, reason, opts) {
  ...
  setTimeout(async function () {
    ...
    try {
      await applyOptionalAudioOutputDevice(media);
...
async function retryTrackSwitchAudioPlayOnce(opts, originalErr, expectedMedia, expectedToken) {
  ...
  await applyOptionalAudioOutputDevice(retryAudio);
...
async function attemptAudioPlay(opts) {
  ...
  try {
    ...
    if (opts.manual || opts.trackSwitch) {
      ...
      await applyOptionalAudioOutputDevice(expectedMedia);
...
function setPlayIcon(p) {
  ...
  if (button) {
    button.dataset.playing = p ? 'true' : 'false';
    button.setAttribute('aria-label', p ? '暂停' : '播放');
  }
...
function pulseCurrentTrackIsLocal() {
  return !!(currentTrack && (
    currentTrack.local === true
    || currentTrack.type === 'local'
    || currentTrack.file
  ));
}

var pulseTransportToggleQueued = false;

function flushPulseTransportToggle() {
  if (!pulseTransportToggleQueued) return;
  pulseTransportToggleQueued = false;
  setTimeout(togglePulseTransportPlayback, 0);
}

function togglePulseTransportPlayback() {
  if (playToggleBusy) {
    pulseTransportToggleQueued = true;
    return Promise.resolve(false);
  }

  var operation;
  if ((!audio || !audio.src) && window.PulseRuntime && PulseRuntime.selection) {
    operation = selectPulseTrack(PulseRuntime.selection.selectedId, {
      origin: 'transport',
      userInitiated: true
    });
  } else {
    operation = togglePlay();
  }

  return Promise.resolve(operation).finally(flushPulseTransportToggle);
}

function selectPulseRelativeTrack(direction) {
  direction = direction < 0 ? -1 : 1;
  if (pulseCurrentTrackIsLocal()) {
    if (direction < 0) prevTrack(true);
    else nextTrack(true);
    return Promise.resolve(true);
  }
  var runtime = window.PulseRuntime;
  var catalog = runtime && runtime.catalog && Array.isArray(runtime.catalog.all)
    ? runtime.catalog.all
    : [];
  if (!catalog.length || !runtime.selection) return Promise.resolve(false);
  var selectedId = runtime.selection.selectedId;
  var index = catalog.findIndex(function (track) { return String(track.id) === String(selectedId); });
  if (index < 0) index = 0;
  var target = catalog[(index + direction + catalog.length) % catalog.length];
  return selectPulseTrack(target.id, {
    origin: 'transport',
    userInitiated: true
  });
}

function syncPulseLoopButton(enabled) {
...
function togglePulseLoop() {
  if (!window.PulseRuntime || !PulseRuntime.playback) return false;
  var enabled = PulseRuntime.playback.setLoop(!(audio && audio.loop));
  if (audio) audio.loop = enabled;
  syncPulseLoopButton(enabled);
  return enabled;
}

function setPulseVolume(value) {
  if (!window.PulseRuntime || !PulseRuntime.playback) return 0;
  var volume = PulseRuntime.playback.setVolume(value);
  if (audio) audio.volume = volume;
  try { localStorage.setItem('pulse-room-volume-v3', String(volume)); } catch (error) { }
  return volume;
}

function bindPulseTransportControls() {
  var playButton = document.getElementById('play-btn');
  var previousButton = document.getElementById('prev-btn');
  var nextButton = document.getElementById('next-btn');
  var playModeButton = document.getElementById('play-mode-btn');
  ...
  if (playButton && playButton.dataset.bound !== 'true') {
    ...
    playButton.addEventListener('click', togglePulseTransportPlayback);
  }
  if (previousButton && previousButton.dataset.bound !== 'true') {
    previousButton.dataset.bound = 'true';
    previousButton.addEventListener('click', function () { selectPulseRelativeTrack(-1); });
  }
  if (nextButton && nextButton.dataset.bound !== 'true') {
    nextButton.dataset.bound = 'true';
    nextButton.addEventListener('click', function () { selectPulseRelativeTrack(1); });
  }
  if (playModeButton && playModeButton.dataset.bound !== 'true') {
    playModeButton.dataset.bound = 'true';
    playModeButton.addEventListener('click', cyclePlayMode);
  }
  if (volumeInput) volumeInput.value = String(audio ? audio.volume : readSavedVolume());
  setPlayIcon(!!(audio && !audio.paused));
  updatePlayModeButton(false);
  return true;
}
